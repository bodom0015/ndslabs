 ...
 */ 
.constant('ApiHost', '141.142.209.135')
...
.value('ApiUri', {
  apiProtocol: 'http://',
  apiHost: '141.142.209.135',
  apiPort: '30001',
  apiPath: '',
  
  // This is cached when the ApiServer builds them up.
  apiFull: '',
})
 ...
 */ 
.factory('NdsLabsApi', [ 'ApiHost', 'ApiPort', 'ApiPath', 'ApiUri', 'WsUri', 'ApiServer', function(ApiHost, ApiPort, ApiPath, ApiUri, WsUri, ApiServer) {
  // TODO: Investigate options / caching
  // XXX: Caching may not be possible due to the unique token sent with every request
  
  // Build up full Api Uri string
  ApiUri.apiFull = ApiUri.apiProtocol + ApiUri.apiHost
      + (ApiUri.apiPort ? ':' + ApiUri.apiPort : '') + ApiUri.apiPath;
      
  // Build up full Websocket Uri string
  WsUri.websocketFull = WsUri.websocketProtocol + WsUri.websocketHost 
      + (WsUri.websocketPort ? ':' + WsUri.websocketPort : '') + WsUri.websocketPath;
  
  return new ApiServer(ApiUri.apiFull);
}])
